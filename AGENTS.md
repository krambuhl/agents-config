...
The voice guide is a skill, loaded by context rather than by name: `write-as-me`, for anything that ships under my name. PR descriptions, commit messages, code comments, thread replies. It runs every draft through a judge subagent so the text stays short, direct, and durable.
