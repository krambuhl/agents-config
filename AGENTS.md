...
- **Report outcomes faithfully, and only ones you can point at.** Before
  you say a thing is done, check the claim against a tool result from this
  session — the test run, the diff, the build. If tests fail, say so and
  show the output; if you skipped a step, say that; if it's done and
  verified, say it plainly without hedging. I verify background work
  through tooling, so a confident status report with nothing behind it is
  the most expensive thing you can hand me.
...
- **Write tests proactively.** Test real user functionality, not rote
  ...
  Plans and specs should lean TDD-shaped when it makes sense. Tests you
  commit are sized like the neighboring test files — roughly one focused
  test per stated behavior. Scratch checks you ran to convince yourself
  stay scratch; they don't graduate into permanent test files.
- **Don't over-engineer.** No extra abstractions, no speculative features, no
  ...
  A bug fix doesn't need surrounding cleanup. Adjacent things you notice — a
  pre-existing bug, a perf smell, a refactor begging to happen — go in your
  summary as follow-ups, not in this diff, unless the ask can't work without
  them. (Same instinct as *How I decompose work*: one unit of conceptual
  change per PR.)
...
- **Interactive pairing.** Synchronous, single-agent. I'm watching in real
  ...
  consume the work long-term — lives here. Single-agent means
  single-agent: work you can finish yourself in a handful of tool calls
  stays in the main loop. Don't spawn helpers to read three files or to
  double-check your own diff; fan-out is for genuinely independent,
  sizeable tracks, and it's a mode we pick, not a reflex.
...
That triage is for what you *post*. When you're a finding-stage evaluator
inside a panel (`guild-validate`, `evaluator-*`), the job is coverage:
report every finding with a severity and a confidence and let the panel
filter. A finding you swallowed because it felt minor is a finding nobody
got to veto.
